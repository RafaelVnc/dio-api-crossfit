...
from dio_fastapi_crossfit.categorias.schemas import CategoriaIn, CategoriaOut
...
async def query(id: UUID4, db_session: DatabaseDependency,) -> list[CategoriaOut]:
    ...
    if not categoria:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="categoria não encontrada no id")
